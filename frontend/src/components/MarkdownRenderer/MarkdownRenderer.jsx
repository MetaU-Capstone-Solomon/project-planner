 ...
 */
const MarkdownRenderer = ({ content, className = '', fallbackToPlain = true }) => {
  ...
  const defaultClasses =
    'prose prose-lg max-w-none prose-headings:text-gray-900 prose-p:text-gray-700 prose-strong:text-gray-900 prose-code:bg-gray-100 prose-code:px-1 prose-code:py-0.5 prose-code:rounded';

  if (!content) {
    return <p className="italic text-gray-500">{MESSAGES.CONTENT.NO_CONTENT_AVAILABLE}</p>;
  }
  ...
  return (
    <pre
      className={`whitespace-pre-wrap rounded-lg border bg-gray-50 p-6 font-sans text-gray-900 ${className}`}
    >
      {content}
    </pre>
  );
