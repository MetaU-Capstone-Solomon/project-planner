...
### Phase 6 — Session Continuity + Project-Level CRUD ✅ COMPLETE (merged to main 2026-04-06)

**New MCP tools:**
- `get_session_handoff({ project_id, last_n_tasks?: 5 })` — N most recently-updated tasks with last note + status; in_progress first
- `delete_project({ project_id, dry_run })` — dry_run preview + hard delete; completes project-level CRUD
- `rename_project({ project_id, new_title })` — title-only update, returns `{ old_title, new_title }`

**Fixes to existing tools:**
- `export_to_cloud` — return payload includes `warning`: local SQLite stale after export, restart in cloud mode
- `scan_repo` — size guard: truncates file contents >200KB, sets `truncated: true`

**Adapter updates:** `deleteProject()` + `renameProject()` on both SqliteAdapter and SupabaseAdapter

**Tests: 99 passing (80 Phase 1–5 + 19 new)**

---

### Phase 7 — Smart Session Handoff ✅ COMPLETE (merged to main 2026-04-07)

**Transactional status + note (`update_task_status` updated):**
- Optional `note` param — status + note in a single DB write; 150 char server-side cap
- `in_progress` note = intent; `completed` note = outcome + what's next

**New tools:**
- `set_project_goal({ project_id, goal })` — permanent `projectGoal` field, never rotated, always in handoff
- `add_session_summary({ project_id, summary })` — appends to `sessions[]`, capped at 10, oldest dropped

**`get_session_handoff` updated:** returns `projectGoal` + `lastSession` + project stats + `recentTasks` (most recent note only per task)

**`mcp-server/CLAUDE.md`:** "continue"/"proceed"/session-open → call `get_session_handoff` first, never ask user to re-explain; session end → call `add_session_summary`

**Tests: 115 passing (99 Phase 1–6 + 16 new)**
...
### All phases 1–7 are complete and merged to main (as of 2026-04-07).

### Phase 8 — (Not yet planned)

Candidates from deferred items:
- **`scan_repo` v2 with Tree-sitter AST** — borrow parser from tirth8205/code-review-graph (MIT licensed); return function signatures + dependency graph instead of raw file contents; richer context, fewer tokens, works on large monorepos
- **Delete stale demo MCP projects** — remove Proplan/ProPlan/ecomerce/test entries from local SQLite, create a proper ProPlan tracking project
- **Local → cloud sync** (paid upsell — requires conflict resolution strategy)
- **Task assignment via MCP** (requires `task_assignments` backend changes)
- **Search/filter tasks by status in MCP**
